...
class TrainingData():
  ...
  DEFAULT_DB_PATH = "../../webserver/trainingdata.db"


  def __init__(self, path=None, transformation_classes=[]):
    self.db_path = path if path else self.DEFAULT_DB_PATH
  ...
  def parse_and_augment_raw_data(self, save_text_output, save_histogram):
    """
...
if __name__ == '__main__':
  ...
  trainer = TrainingData(path=path, transformation_classes=transformation_classes)

  # Augment all images. Saving histogram is slow, and they are only for the
  # end user, so generating them is omitted here.
  data = trainer.parse_and_augment_raw_data(save_text_output=True, save_histogram=False)